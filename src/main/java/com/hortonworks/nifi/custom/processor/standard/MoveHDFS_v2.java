...
import org.apache.nifi.processors.hadoop.*;
...
public class MoveHDFS_v2 extends AbstractHadoopProcessor {
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        //final String destinationFolder = context.getProperty(DIRECTORY).evaluateAttributeExpressions(parentFlowFile).getValue();
    ...
    protected void processBatchOfFiles(final List<Path> files, final ProcessContext context,
        ...
        int counter = 1;
        final String flowfile_filename = parentFlowFile.getAttribute("filename").toString();
        final String flowfile_uuid = parentFlowFile.getAttribute("uuid").toString();
            ...
            ugi.doAs(new PrivilegedAction<Object>() {
                ...
                public Object run() {
                    ...
                    try {
                        ...
                        final Path newFile = new Path(destinationFolder, originalFilename);
                        ...
                        //Defragments
                        //--------------
                        int file_index = files.indexOf(file);
                        flowFile = session.putAttribute(flowFile, "fragment.count", String.valueOf(files.size()));
                        flowFile = session.putAttribute(flowFile, "fragment.index", String.valueOf(file_index));
                        flowFile = session.putAttribute(flowFile, "segment.original.filename", flowfile_filename);
                        flowFile = session.putAttribute(flowFile, "filename", flowfile_filename);
                        flowFile = session.putAttribute(flowFile, "fragment.identifier", flowfile_uuid);
                        flowFile = session.putAttribute(flowFile, "fragment.size", "0");
    ...
    protected static class ProcessorConfiguration {
        ...
        ProcessorConfiguration(final ProcessContext context) {
            ...
            final String dirValue = context.getProperty(DIRECTORY).evaluateAttributeExpressions().getValue();


            outputRootDirPath = new Path(dirValue);
